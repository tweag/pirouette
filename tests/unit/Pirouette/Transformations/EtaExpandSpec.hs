...
tests =
  [ testCase "add ~ \\x y -> add x y" $
    ...
    testCase "const ~  /\\(a : *) (b : *) . \\(x : a) (y : b) . const @a @b x y" $
      [term| const |] `isEtaEq` [term| /\(a : *) (b : *) . \(x : a) (y : b) . const @a @b x y |],
    testCase "omg @Integer 42 ~ \\a : Integer . /\\ f : (* -> *) . \\(x : f Integer) . omg @Integer 42 a @f x" $
      [term| omg @Integer 42 |]
        `isEtaEq` [term| \a : Integer . /\ f : (* -> *) . \(x : f Integer) . omg @Integer 42 a @f x |]
  ]
