...
checkDefsToSMT :: (PirouetteReadDefs lang m, LanguagePretty lang, LanguageSMT lang) => m ()
checkDefsToSMT = do
  allDefs <- prtAllDefs
...
translateTerm Raw.Lam {} = fail "Translate term to smtlib: Lambda abstraction in term"
translateTerm Raw.Abs {} = fail "Translate term to smtlib: Type abstraction in term"
...
translateVar (Raw.Bound (Raw.Ann _name) _) = fail "translateVar: Bound variable; did you forget to apply something?"
...
constructorFromPIR (name, constructorType) =
  (toSmtName name,)
    ...
    zip
      ((toSmtName name ++) . ("_" ++) . show <$> [1 :: Integer ..])
    <$> aux constructorType
  where
