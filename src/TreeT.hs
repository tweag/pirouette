...
data Branch tag state a = Branch {tag :: tag, next :: Tree tag state a}
  deriving (Eq, Show, Functor, Foldable)
...
{-# SPECIALIZE runTreeAny ::
  forall tag state a r.
  (a -> Maybe r) ->
  TreeT tag state Identity a ->
  state ->
  Maybe r
